classdef CKDataObjectIMAGE < BrukerDataSuperclass
...
% readBrukerRaw, convertRawToFrame, convertFrameToCKDataIMAGE,
% readBruker2dseq
...
methods
    ...
    %% Constructor
    function obj=CKDataObjectIMAGE(varargin)
    ...
    function obj=calcKData(obj, frameMatrix, varargin)
        ...
        obj.data = convertFrameToCKDataIMAGE( frameMatrix, obj.Acqp,  obj.Method, 'specified_NRs', specified_NRs, 'useMethod', obj.useMethod);         
    end        
    ...
    function out_obj=reco(obj, recopart, varargin)
        ...
        %                 required parameters: RECO_ft_size.
        %                 optional parameter: signal_position.
        ...
        %   'scale_phase_channels': apply the scaling and phase offset for each receive channel, as set in the coil configuration.
        %                 required parameters: RecoScaleChan, RecoPhaseChan
        ...
        %   'all' : equivalent to recopart={'quadrature', 'phase_rotate', 'zero_filling', 'FT', 'phase_corr_pi', ...
        ...
        %
        %     signal_position - column-vector with NumberOfDimensions elements (maximum: 4) between 0 and 1. It defines the position of your data between the zeros.
        ...
        %
        %     RecoScaleChan, RecoPhaseChan - double arrays (one element per active receive channel).
        %         RecoScaleChan stores the scaling factors that need to be applied to each receive channel in order to obtain a similar signal
        %         level in all channels. Usually, these factors are all equal or close to 1.0.
        %         RecoScaleChan stores the phase angles (in degrees) that need to be applied to each receive channel in order to obtain a
        %         similar image phase in all channels (only relevant in real, imaginary, and phase images). For example, for the two channels
        %         of a quadrature coil, the angles read 0.0 and 90.0 degrees.
        ...
        if bruker_getAcqPvVersion(obj.Acqp,'is360')
            for dim = 1:length(obj.Reco.RECO_ft_mode)
                if strcmp(obj.Reco.RECO_ft_mode{dim}, 'COMPLEX_FT') || strcmp(obj.Reco.RECO_ft_mode{dim}, 'COMPLEX_FFT')
                    obj.Reco.RECO_ft_mode{dim} = 'COMPLEX_IFT';
                else
                    obj.Reco.RECO_ft_mode{dim} = 'COMPLEX_FT';
                end
            end
        end
