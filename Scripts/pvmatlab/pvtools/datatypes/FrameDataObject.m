...
% FrameDataOobject
% This class can create and store frame data and store parameter structs
% 
...
%       -> Does: Imports a rawDataObject & converts the fid-Matrix to a frameMatrix
...
%       -> Requires: Standard ParaVision directory structure
%       -> Does: Directly creates a FrameDataObject
%
% Parameters
%
%    Required
...
%          You only need to insert it, IF you read your fidFile with specified_NRs AND the rawDataClass didn't store your specified_NRs in the .Acqp-struct
%          Keep in mind, the first NR loop starts with an 1
%          e.g. obj = FrameDataObject('../Testfiles/10', 'specified_NRs', [1 5 7] );
%      ['specified_Job', the_chosenJob] - (2.) & (3.) 
%          Type: defined string followed by an integer value, default is 1 (= read the first job in the cellstruct)
%          Why use: Starting from ParaVision 6, a new kind of data acquisition is available, so-called "job acquisition". Normally you work with the fid-file, 
%          this is stored (if it exists) in rawDataObject.data{1} (=default), but sometimes it can be necessary to choose another data matrix
...
methods
    ...
    function obj=calcFrameData(obj, rawMatrix, varargin)
        ...
        obj.data = convertRawToFrame( rawMatrix, obj.Acqp, obj.Method, 'specified_NRs', specified_NRs);           
    end
