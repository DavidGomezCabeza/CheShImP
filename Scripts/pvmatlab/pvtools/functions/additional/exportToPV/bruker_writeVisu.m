...
%     FileID=fopen(join([pwd(), "\pvmatlab\", writepath, filesep, 'visu_pars'],''),'wb');
    FileID=fopen(join([writepath, filesep, 'visu_pars'],''),'wb');
...
%% Header
    ...
    if isfield(exportVisu, 'VisuCoreOrientation') && ~isempty(exportVisu.VisuCoreOrientation)
        ...
        for i=1:length(VisuCoreOrientation)
            tmp=[tmp, num2str(VisuCoreOrientation(i),15), ' '];
        end
    ...
    if  isfield(exportVisu, 'VisuCorePosition') && ~isempty(exportVisu.VisuCorePosition)    
        ...
        for i=1:length(VisuCorePosition)
            tmp=[tmp, num2str(VisuCorePosition(i),15), ' '];
        end
    ...
    % VisuSubjectType
    if isfield(exportVisu, 'VisuSubjectType') && ~isempty(exportVisu.VisuSubjectType)
        fprintf(FileID, '%s\n', ['##$VisuSubjectType=' exportVisu.VisuSubjectType]);
    end
    ...
    % VisuStudyDate
    if isfield(exportVisu, 'VisuStudyDate') && ~isempty(exportVisu.VisuStudyDate)
        fprintf(FileID, '%s\n', ['##$VisuStudyDate=<' exportVisu.VisuStudyDate '>']);
    end
    ...
    % VisuSeriesFrameOfReferenceUid
    if isfield(exportVisu, 'VisuSeriesFrameOfReferenceUid') && ~isempty(exportVisu.VisuSeriesFrameOfReferenceUid)
        fprintf(FileID, '%s\n', '##$VisuSeriesFrameOfReferenceUid=( 65 )');
        fprintf(FileID, '%s\n', ['<', exportVisu.VisuSeriesFrameOfReferenceUid, '>']);
    end
