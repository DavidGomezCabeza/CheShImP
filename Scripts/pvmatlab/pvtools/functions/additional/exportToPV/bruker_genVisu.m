...
if (strcmpi(computer('arch'), 'glnx86') || strcmpi(computer('arch'), 'glnxa64')) || sum(strfind(computer, 'linux'))>=1
    % Linux
    % linux-command to grep the mac-adresses
    [dummy, mac]=system('/sbin/ifconfig | grep -o -E ''([[:xdigit:]]{1,2}:){5}[[:xdigit:]]{1,2}''');
    point_pos=strfind(mac, ':');
    mac(point_pos)='';
    mac=mac(1:12); % get rid of spaces, take first mac-address
    try
        host_identifier=num2str(hex2dec(mac));
    catch
        error('The mac-adress of your system can''t be acquired automatically.');
    end
else
    % Windows/Mac
    % host_identifier=hostid; %  MATLAB hostid command is deprecated
    [dummy,getmac_str] = dos('getmac');
    [mac_start_idx, mac_end_idx] = regexp(getmac_str,'([0-9A-F]{2}[:-]){5}([0-9A-F]{2})');
    getmac_str = getmac_str(mac_start_idx:mac_end_idx);
    getmac_str = strrep(getmac_str,'-','');
    host_identifier = num2str(hex2dec(getmac_str));
end
...
if sum(strcmpi(varargin, 'importVisuCoreOrientation'))>=1  || sum(strcmpi(varargin, 'importAll'))>=1 
    ...
    lenFGSlice = 0;
    if isfield(importVisu,'VisuFGOrderDescDim') && isfield(importVisu,'VisuFGOrderDesc')
        for frameGroup = 1:importVisu.VisuFGOrderDescDim
          if isequal(importVisu.VisuFGOrderDesc{2,frameGroup},'FG_SLICE')
              lenFGSlice = importVisu.VisuFGOrderDesc{1,frameGroup};
              break;
          end
       end
    end
    ...
    elseif exist('VisuCoreOrientation', 'var') && (numel(VisuCoreOrientation)==9 || numel(VisuCoreOrientation)==9*exportVisu.VisuCoreFrameCount ...
            || (lenFGSlice > 0 && numel(VisuCoreOrientation)==9*lenFGSlice))
...
if sum(strcmpi(varargin, 'importVisuCorePosition'))>=1  || sum(strcmpi(varargin, 'importAll'))>=1
    ...
    elseif exist('VisuCorePosition', 'var') && (numel(VisuCorePosition)==3 || numel(VisuCorePosition)==3*exportVisu.VisuCoreFrameCount ...
            || (lenFGSlice > 0 && numel(VisuCorePosition)==3*lenFGSlice))
...
switch genmode
    case {'auto'}
        if isfield(Template, 'VisuSubjectName') && ~isempty(Template.VisuSubjectName)
            exportVisu.VisuSubjectName=Template.VisuSubjectName;
    ...
    case 'Visu'
        if isfield(importVisu, 'VisuSubjectName') && ~isempty(importVisu.VisuSubjectName)
            exportVisu.VisuSubjectName=importVisu.VisuSubjectName;
        else
            exportVisu.VisuSubjectName='anonymus';
        end
    case 'Subject'
...
%% VisuSubjectType
if strcmp(genmode, 'Template') && isfield(Template, 'VisuSubjectType') && ~isempty(Template.VisuSubjectType)
    exportVisu.VisuSubjectType = Template.VisuSubjectType;
end

%% VisuStudyDate
if strcmp(genmode, 'Template') && isfield(Template, 'VisuStudyDate') && ~isempty(Template.VisuStudyDate)
    exportVisu.VisuStudyDate = Template.VisuStudyDate;
end

%% VisuSeriesFrameOfReferenceUid
if strcmp(genmode, 'Template') && isfield(Template, 'VisuSeriesFrameOfReferenceUid') && ~isempty(Template.VisuSeriesFrameOfReferenceUid)
    exportVisu.VisuSeriesFrameOfReferenceUid = Template.VisuSeriesFrameOfReferenceUid;
end
