...
%                         required parameter: RECO_ft_size.
...
% Individual scaling and phasing of the receive channels
[varargin, RecoScaleChan] = bruker_addParamValue(varargin, 'RecoScaleChan', '@(x) isnumeric(x)', []);
if ~isempty(RecoScaleChan), Reco.RecoScaleChan=RecoScaleChan; end
...
for part=1:length(recopart)
    switch recopart{part}
        ...
        case 'scale_phase_channels'
            warning('off','backtrace')
            for NR=1:size(data,7)
                for NI=1:size(data,6)
                    for channel=1:size(data,5)
                        [ data(:,:,:,:,channel,NI,NR) ] = reco_scale_phase_channels( data(:,:,:,:,channel,NI,NR) , Reco, channel );
                    end
                end
            end
            warning('on','backtrace');
            warning('on','MATLAB:bruker_RecoScaleChan');
            warning('on','MATLAB:bruker_RecoPhaseChan');
...
for i=1:length(RECO_qopts)
   switch(RECO_qopts{i})
       case 'COMPLEX_CONJUGATE'
           frame=conj(frame); % David: Pluses become minus and viceversa. 
       case 'QUAD_NEGATION'
           switch i
               case 1
...
%% Added by David
% Reco.RECO_ft_size(1) = Reco.RECO_ft_size(1)*2;
%%
